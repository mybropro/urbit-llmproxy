...
+$  pending-client
  $:  eyre-id=@ta
      ...
      started=@da
  ==
::  Older pending-client shape, kept for state-0 deserialization.
+$  pending-client-0
  $:  eyre-id=@ta
      target=@p
      model=@t
  ...
  =/  migrated=state-1
    ...
    ::  init telemetry empty.
      ...
      :*  %1
          ...
          client-api-token.u.loaded
          policy.u.loaded
          hosting.u.loaded
          *(map @ud pending-client)
          pending-config.u.loaded
...
++  on-poke
  ...
  ?+    mark  (on-poke:def mark vase)
    ...
    ::
      ...
      =/  rec=pending-client
        [eyre-id='' target.cmd model.cmd stream=%.n %dojo prompt.cmd api-base='' started=now.bowl]
      :_  %=  this
              nonce    n
  ...
  ::
    ...
    ?:  ?&  ?=(%'GET' method.request)
      ...
      ?.  (bearer-ok client-api-token.state header-list.request)
        ...
        :_  this(telemetry (scag telemetry-cap [entry telemetry.state]))
      ...
      :_  this(models fresh-models, telemetry (scag telemetry-cap [entry telemetry.state]))
      %+  give-simple-payload:app:server  eyre-id
      ...
      =/  new-telemetry=(list client-telemetry-entry:llmproxy)
        ...
        =/  entry=client-telemetry-entry:llmproxy
          :*  now.bowl
              ...
              (met 3 err-body)
          ==
        (scag telemetry-cap [entry telemetry.state])
